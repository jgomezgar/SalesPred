/*############################## 9 min #######################################*/
-- OUTPUT: [STAGING_2].[dbo].[XXX_P_Sell_IN_Direct_Activities_10d_fit]
...
SET @t1 = GETDATE();
/*#########################################################################*/
IF OBJECT_ID('[STAGING_2].[dbo].[XXX_P_Sell_IN_Direct_Activities_10d_fit]', 'U') IS NOT NULL
 DROP TABLE [STAGING_2].[dbo].[XXX_P_Sell_IN_Direct_Activities_10d_fit];
...
FROM [STAGING_2].[dbo].[XXX_P_Sell_IN_Direct_Activities_10d] s 
)
...
into [STAGING_2].[dbo].[XXX_P_Sell_IN_Direct_Activities_10d_fit]
from repe r
left join repe r1 
      on r.r = r1.r+1 
...
left join 
[STAGING_2].[dbo].[XXX_P_TOTAL_Sell_IN_Direct_Activities_10d] total on 
...
select s.[CUSTOMER_ID], sum(SI_ITG_WSE) SI_ITG_WSE,	SI_MRKT_WSE
 from [STAGING_2].[dbo].XXX_P_Sell_IN_Direct_Periods_10d s 
...
SET @t2 = GETDATE();
SELECT '06.P_Sell_IN_Direct_Activities_Fit.sql' as SCRIPT,
